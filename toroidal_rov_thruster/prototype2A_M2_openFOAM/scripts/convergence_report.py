...
Signs (CFD frame, rotation axis +x, jet designed toward +x):
  Fx      signed axial force of the water on rotor, stator and assembly. A +x jet
...
def main():
    ...
    rf, rm, df = (read_table('rotorForces', 'force.dat'), read_table('rotorForces', 'moment.dat'),
                  read_table('statorForces', 'force.dat'))
    ...
    series = [('Assembly Fx (N)', t, fx_total, True),
              ...
              ('Stator Fx, duct + support (N)', t, fx_duct, False),
    ...
    if args.write:
        mesh = mesh_stats()
        stl = next((p for p in (ROOT / 'constant' / 'triSurface' / 'rotor_p2a.stl',
                                ROOT / 'geometry' / 'rotor_p2a.stl') if p.exists()), None)
        digest = hashlib.sha256(stl.read_bytes()).hexdigest() if stl else 'n/a'
        md = ['# Prototype 2A-M2 L0 run', '',
              'Steady MRF, k-omega SST, bollard (still water), seawater, 3000 rpm, forward rotation. '
              ...
              f'- Rotor STL sha256 `{digest}`; see geometry/cfd_geometry.json', '',
              '## Result', '', '```', *lines, '```', '']
